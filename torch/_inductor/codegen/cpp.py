    ...
    def legalize_lowp_fp_dtype_loopbody(self, loop_body: LoopBody):
        def add_to_dtype(sub_graph: torch.fx.Graph):
            def is_lowp_fp_source(node: torch.fx.Node):
                """ Given its ouput is the input of a node that requires lowp fp input,
                check if the node can generates lowp fp ouput.
                """
                dtype_sources = [
                    "load",
                    "constant",
            ...
            def is_lowp_fp_sink(node: torch.fx.Node):
                """ Given its input is the output of a node that generates lowp fp output,
                check if the node can accepts lowp fp input.
                """
                dtype_sinks = [
                    "store",
                    "to_dtype",
            ...
            for _node in sub_graph_nodes:
                if is_lowp_fp_load(_node):
                    ...
                    # No need to promote to float if it is a user of a lowp fp sources which are all directly fed to ops that accepts lowp fp input
                    ...
                    if dtype in DTYPE_LOWP_FP:
                        # No need to promote to float if all users are ops that accepts lowp fp input
                        if not (
                            all(is_lowp_fp_sink(user) for user in _node.users)
                        ):
                            ops = _node.args[0]
                            with sub_graph.inserting_after(_node):
                                to_type_node = sub_graph.call_method(

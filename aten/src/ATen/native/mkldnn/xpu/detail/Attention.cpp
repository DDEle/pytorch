...
at::Tensor expand_to_4d(at::Tensor tensor) {
  for (size_t i = tensor.dim(); i < 4; i++) {
    tensor = at::unsqueeze(tensor, 0);
  }
  return tensor;
}
} // namespace
...
void gpu_float_sdpa(
  ...
  if (attn_mask.has_value()) {
    attn_mask = expand_to_4d(*attn_mask);
  }
  ...
  if (attn_mask.has_value()) {
    ...
    const bool support_implict_causal = driver_support_implict_causal &&
        query.dtype() == at::kHalf && seq_len_q == seq_len_k;
